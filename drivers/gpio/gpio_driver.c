...
void set_pin_mode(GPIO_t *gpio_port, uint8_t pin, GPIO_pin_mode_t mode){
    ((GPIO_t *)gpio_port)->MODER &= ~(0x03 << (pin * 2)); // Clear the mode bits for the specified pin
    ((GPIO_t *)gpio_port)->MODER |= (mode << (pin * 2)); // Set the new mode for the specified pin
}

void set_pin_output_type(GPIO_t *gpio_port, uint8_t pin, GPIO_pin_output_type_t output_type){
    ((GPIO_t *)gpio_port)->OTYPER &= ~(0x01 << pin); // Clear the output type bit for the specified pin
    ((GPIO_t *)gpio_port)->OTYPER |= (output_type << pin); // Set the new output type for the specified pin
}

void set_pin_speed(GPIO_t *gpio_port, uint8_t pin, GPIO_pin_speed_t speed){
    ((GPIO_t *)gpio_port)->OSPEEDR &= ~(0x03 << (pin * 2)); // Clear the speed bits for the specified pin
    ((GPIO_t *)gpio_port)->OSPEEDR |= (speed << (pin * 2)); // Set the new speed for the specified pin
}

void set_pin_pull(GPIO_t *gpio_port, uint8_t pin, GPIO_pin_pull_t pull){
    ((GPIO_t *)gpio_port)->PUPDR &= ~(0x03 << (pin * 2)); // Clear the pull-up/pull-down bits for the specified pin
    ((GPIO_t *)gpio_port)->PUPDR |= (pull << (pin * 2)); // Set the new pull-up/pull-down configuration for the specified pin
}
...
void set_pin_state(GPIO_t *gpio_port, uint8_t pin, GPIO_pin_set_reset_t set_reset){
    if(set_reset == SET){
        ((GPIO_t *)gpio_port)->BSRR = (1UL << pin); // Set the set bit for the specified pin
    } else{
        ((GPIO_t *)gpio_port)->BSRR = (1UL << (pin + 16)); // Set the reset bit for the specified pin
    }
}
